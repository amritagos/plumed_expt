    ...
    * If the are commands directly sent to the LAMMPS object this may
    ...
    def initialise_lammps(self, atoms):
        ...
        if self.parameters.create_box:
            ...
            if self.parameters.n_bond_types == 0 and self.parameters.n_angle_types == 0:
                create_box_command = f"create_box {n_types} cell"
            else:
                create_box_command = f"create_box {n_types} cell bond/types {self.parameters.n_bond_types} angle/types {self.parameters.n_angle_types} extra/bond/per/atom {self.parameters.bonds_per_atom} extra/angle/per/atom {self.parameters.angles_per_atom}"
        ...
        # Create the bonds and angles once
        if self.parameters.bond_angle_creation is not None:
            for cmd in self.parameters.bond_angle_creation:
                self.lmp.command(cmd)
