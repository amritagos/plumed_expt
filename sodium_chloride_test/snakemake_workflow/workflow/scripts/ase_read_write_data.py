...
def read_lammps_data(
    ...
    for line in fileobj:
        ...
        if match is not None:
            ...
            if section == "Atoms":  # id *
                ...
                mol_id_in, type_in, charge_in, pos_in, travel_in = _read_atoms_section(
                    fileobj, natoms, atom_style
                )
            continue
    ...
    # Shift all atoms by xlo, ylo and zlo, so that the box starts from 0.0, 0.0, 0.0
    pos = atoms.get_positions()
    new_pos = pos - np.array([xlo, ylo, zlo])
